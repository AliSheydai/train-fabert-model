...
def print_banner():
    ...
    print("   FaBERT SENTIMENT ANALYZER  |  XAI PROJECT DEMO")
...
print(Colors.YELLOW + f"[SYSTEM] Loading Model from: {model_path}" + Colors.RESET)

# --- 2. LOAD MODEL ---
try:
    # device=-1 for CPU
    classifier = pipeline("sentiment-analysis", model=model_path, tokenizer=model_path, device=-1)
    # Fake loading delay just for a cool effect in the video/screenshot (Optional)
    time.sleep(1) 
    print(Colors.GREEN + "[SUCCESS] Model Loaded Successfully! Engine Ready." + Colors.RESET)
    print("-" * 60)
...
def predict(text, auto_mode=False):
    ...
    if auto_mode:
        time.sleep(0.5) # Pause slightly between auto examples

# --- 4. RUN DEMO ---
print(Colors.CYAN + "\n>>> RUNNING AUTOMATED DIAGNOSTICS (Fabert EXAMPLES):" + Colors.RESET)
test_sentences = [
    # --- (Easy Positive) ---
